...
# --- DEBUGGING: Initial script load time ---
start_time = datetime.now()
st.info(f"[DEBUG] Script started at: {start_time.strftime('%H:%M:%S')}")
...
# Load Data
current_time_data_load = datetime.now()
st.info(f"[DEBUG] Starting data loading at: {current_time_data_load.strftime('%H:%M:%S')} ({(current_time_data_load - start_time).total_seconds():.2f}s after script start)")
with st.spinner("📡 Loading real-time nominations & inventory..."):
    ...
    rigid_surcharge = CONFIG["pricing"]["route_flex_discount"]
...
current_time_data_loaded = datetime.now()
st.info(f"[DEBUG] Data loading completed at: {current_time_data_loaded.strftime('%H:%M:%S')} ({(current_time_data_loaded - current_time_data_load).total_seconds():.2f}s for data load)")
...
if run_btn:
    current_time_opt_start = datetime.now()
    st.info(f"[DEBUG] Starting optimization at: {current_time_opt_start.strftime('%H:%M:%S')}")
    with st.spinner("🧠 Running GNN-MILP Hybrid Solver... Please wait."):
        ...
        current_time_opt_end = datetime.now()
        st.info(f"[DEBUG] Optimization completed at: {current_time_opt_end.strftime('%H:%M:%S')} ({(current_time_opt_end - current_time_opt_start).total_seconds():.2f}s for optimization)")
        st.rerun()
